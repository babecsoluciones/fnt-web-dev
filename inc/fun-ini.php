...
{
    $select = "SELECT * FROM CatRedesSociales ORDER BY eCodRedSocial ASC";
    $rsRedes = mysql_query($select);
    while($rRed = mysql_fetch_array($rsRedes))
    { ?>
       <a href="<?=$rRed{'tEnlace'};?>"><span class="<?=$rRed{'tIcono'};?>"></span></a> 
    <? }
...
function galeria()
{
    $select = "SELECT * FROM CatImagenes ORDER BY RAND() LIMIT 0,32";
    $rsImagenes = mysql_query($select);
    while($rImagen = mysql_fetch_array($rsImagenes))
    { ?>
...
{
    $i=0;
    $select = "SELECT * FROM CatImagenes WHERE tCodPaquete = '$codigo' ORDER BY RAND()  LIMIT 0,6";
    $rsImagenes = mysql_query($select);
    while($rImagen = mysql_fetch_array($rsImagenes))
    { ?>
